...
class RoleDetail extends Component {
  ...
  handleSubmit() {
    this.props.form.validateFieldsAndScroll((err, values) => {
      ...
      // data.content.goods_config.address = model.address;

      // data.key = 'lalalssssunweixing';
      // data.valid_at = '2018-9-14';
      // console.log(data);
      this.doSubmit(data);
    });
  }
  ...
  doSubmit(values) {
    ...
    let success = false; //success:false
    const cbs = {
      success: (res, data) => {
        success = true;
  ...
  handleChangeInputLength(e, len) {
    ...
    // e.target.addEventListener('compositionstart', () => {
    //   console.log(111);
    //   this.setState({
    //     p: false,
    //   });
    // });
    // e.target.addEventListener('compositionend', () => {
    //   console.log(222);
    //   this.setState({
    //     p: true,
    //   });
  ...
  render() {
    ...
    const { getFieldDecorator } = this.props.form;
    const { model, value, ifInput } = this.state;
    console.log(model);
      ...
      <div className="baseinfo-wrapper">
        <Form>
          ...
          <FormItem label="课程名称:" {...formItemLayout}>
            ...
            })(<Input onChange={e => this.handleChangeInputLength(e, 20)} />)}
              </Col>
              <Col span={1} />
          ...
          <FormItem label="课程地址:" {...formItemLayout}>
            <Row>
              <Col span={18}>{getFieldDecorator('address', {//description(可更改)
              initialValue: model.address,
              rules: [{ required: true, message: '请填写课程地址' }],
            })(<Input type="textarea"
              onChange={e => this.handleChangeInputLength(e, 100)}
            />)}
              </Col>
              <Col span={1} />
              <Col span={2}><span style={addressLen > 100 ? { color: '#f00' } : {}}>{addressLen}</span>/100</Col>
            </Row>
          </FormItem>
          <FormItem label="购买后短信文案:" {...formItemLayout}>
            <Row>
              <span>恭喜您预订成功[课程名]！名额[数量]位，活动当天凭二维码入场。</span>
              <Col span={18}>{getFieldDecorator('sms', {//description(可更改)
          ...
          <FormItem label="商品价格名称:" {...formItemLayout}>
            <Row>
              <Col span={18}>{getFieldDecorator('session', {
              initialValue: model.session,
              rules: [{ required: true, message: '请填写价格名称' }],
            })(<Input
